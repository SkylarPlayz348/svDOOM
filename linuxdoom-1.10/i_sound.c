                 ...
                 int priority)
{
  ...
  id = addsfx(id, 127, steptable[pitch], sep);
...
{
  if (audio_stream && SDL_GetAudioStreamQueued(audio_stream) <= SAMPLECOUNT * BUFMUL * 2)
    SDL_PutAudioStreamData(audio_stream, mixbuffer, SAMPLECOUNT * BUFMUL);
}
...
void I_InitSound()
{
  ...
  // Open SDL3 audio stream: signed 16-bit LE, stereo, 11025 Hz.
